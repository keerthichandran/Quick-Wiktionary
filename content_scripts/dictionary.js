...
function showMeaning() {
    ...
    // url = 'https://test.wikipedia.org/w/api.php?action=query&meta=token&type=login&format=json'
    ...
    fetchRes.then((response) => response.status === 200 && response.json())
        .then((response) => {
            ...
            if (!response.extract || response.extract.split('may refer to:')[1] === "" || response.extract.split('may refer to:')[1] === "") return noMeaningFound(createdDiv, word);
            appendToDiv(createdDiv, {
                word: word,
                meaning: response.extract,
                image: response.thumbnail !== undefined ? response.thumbnail.source : null
            });
        })
        .catch((error) => {
...
function noMeaningFound(createdDiv, word) {
    ...
    createdDiv.meaning.style = "color: #aaa;"
